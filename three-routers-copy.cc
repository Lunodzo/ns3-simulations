...
void CheckQueueSize(std::string context, uint32_t before, uint32_t after){
    std::cout << YELLOW_CODE << context << END_CODE << std::endl;
}

void BackoffTrace(std::string context, Ptr<const Packet> packet){
    std::cout << YELLOW_CODE << context << END_CODE << std::endl;
    EthernetHeader hdr;
...
int main(int argc, char *argv[])
{
    ...
    //Attaching CsmaNetDevice to all LAN 1 nodes.
    ...
    //Install a UdpEchoServer on all nodes of LAN2
